...
bool drain_socket(Connection &conn) {
  while (true) {
    ...
    if (write_space == 0) {
      std::cerr << "Ring buffer full! Need larger buffer or faster processing."
                << std::endl;
      return false;
    }
...
int main() {
  ...
  std::cout << "Using epoll (Linux) with zero-copy ring buffer" << std::endl;
  ...
  std::cout << "Using kqueue (macOS/BSD) with zero-copy ring buffer"
            << std::endl;
#endif
