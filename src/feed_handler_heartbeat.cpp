...
public:
  FeedHandler(const std::string& host, int port)
    : conn_manager_(host, port)
    , should_stop_(false)
    , stats_()
  {}
  
  void run() {
    std::cout << "=== Exercise 7: Feed Handler with Heartbeat & Reconnection ===" << std::endl;
    ...
    while (!should_stop_) {
      ...
      if (conn_manager_.is_heartbeat_timeout()) {
        std::cout << "[FeedHandler] ⚠️  Heartbeat timeout detected ("
                  << conn_manager_.seconds_since_last_message() << "s since last message)"
                  << std::endl;
        ...
        if (conn_manager_.reconnect()) {
          ...
          std::cerr << "[FeedHandler] Reconnection failed, retrying..." << std::endl;
          continue;
        }
      }
...
private:
  bool read_and_process() {
    ...
    ssize_t bytes_read = recv(conn_manager_.sockfd(), write_ptr, write_space, 0);
  ...
  void process_messages() {
    while (true) {
      ...
      // Validate header
      if (header.type != MessageType::TICK && header.type != MessageType::HEARTBEAT) {
  ...
  void process_heartbeat(const MessageHeader& header, const char* payload) {
    ...
    std::cout << "[Heartbeat seq=" << header.sequence << "] timestamp=" 
              << heartbeat.timestamp << std::endl;
  }
