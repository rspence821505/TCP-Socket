...
public:
  ReaderThread(int sockfd, SPMCQueue<TimedMessage>& queue, std::atomic<bool>& stop_flag)
...
public:
  ConsumerThread(int id, SPMCQueue<TimedMessage>& queue, 
    ...
    : consumer_id_(id), queue_(queue), reader_done_(reader_done), stats_(stats) {}
  
  void run() {
    std::cout << "[Consumer " << consumer_id_ << "] Thread started" << std::endl;
      ...
      if (msg) {
        ...
        if (local_messages % 20000 == 0 && local_messages > 0) {
          ...
          std::cout << "[Consumer " << consumer_id_ << "] [" << symbol << "] $" 
                    << msg->tick.price << " @ " << msg->tick.volume << std::endl;
        }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "=== Exercise 5 Extension: SPMC Feed Handler ===" << std::endl;
  std::cout << "Configuration:" << std::endl;
  std::cout << "  Queue capacity: " << queue_size << std::endl;
  std::cout << "  Number of consumers: 2" << std::endl;
  std::cout << "  Cache-line padding: " << (use_padding ? "ENABLED" : "DISABLED") << std::endl;
  std::cout << std::endl;
  
  if (!use_padding) {
    std::cout << "⚠️  WARNING: Running WITHOUT padding - expect false sharing!" << std::endl;
  ...
  if (use_padding) {
    ...
    std::thread consumer1([&]() {
      ConsumerThread<ConsumerStatsPadded> consumer(0, queue, reader_done, stats[0]);
      consumer.run();
    });
    
    std::thread consumer2([&]() {
      ConsumerThread<ConsumerStatsPadded> consumer(1, queue, reader_done, stats[1]);
      consumer.run();
    });
    ...
    uint64_t total_messages = stats[0].messages_processed + stats[1].messages_processed;
    uint64_t total_latency = stats[0].total_latency_ns + stats[1].total_latency_ns;
    
    std::cout << "\n=== Statistics ===" << std::endl;
    std::cout << "Consumer 0: " << stats[0].messages_processed << " messages" << std::endl;
    std::cout << "Consumer 1: " << stats[1].messages_processed << " messages" << std::endl;
    std::cout << "Total: " << total_messages << " messages in " << seconds << "s ("
              << static_cast<int>(total_messages / seconds) << " msgs/sec)" << std::endl;
    std::cout << "Average latency: " << (total_latency / total_messages / 1000.0) 
              << " µs" << std::endl;
    ...
    std::thread consumer1([&]() {
      ConsumerThread<ConsumerStatsUnpadded> consumer(0, queue, reader_done, stats[0]);
      consumer.run();
    });
    
    std::thread consumer2([&]() {
      ConsumerThread<ConsumerStatsUnpadded> consumer(1, queue, reader_done, stats[1]);
      consumer.run();
    });
